...
def process_files(args):
    def process(files_chunks, do_map):
        ...
        for files_chunk in files_chunks:
            for f, (num_records, mean_len_record) in zip(
                files_chunk,
                do_map(
                    process_file,
                    [
                        (args, to_local_in_file(f), to_local_out_file(f))
                ...
                print(
                    f"file={f} mean_len_record={int(mean_len_record)} num_records={num_records} total_num_records={total_num_records}",
                    flush=True,
                )
                total_num_records += num_records
                total_num_files += 1
        print(
            f"finished {total_num_files} files and {total_num_records} total records",
            flush=True,
        )
