...
def process_file(args, file):
    ...
    print(
        f"finalizing file {out_file_tmp} -> {out_file} with {total_records_out} records",
        flush=True,
    )
    ...
    print(
        f"finished total_records_in={total_records_in} total_records_out={total_records_out}"
    )
...
def main():
    ...
    with print_time(f"hashing files with {args.n_threads} threads"):
        with ThreadPool(args.n_threads) as pool:
            for total_records_in_lang, total_records_out_lang in pool.starmap(
                process_file, list(map(lambda l: (args, l), files))
            ):
    ...
    print(
        f"total_records_in={total_records_in} total_records_out={total_records_out}",
        flush=True,
    )
