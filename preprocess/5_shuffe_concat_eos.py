...
def yield_ds(args, files):
    ...
    def append_bos_fn(sample):
        return tf.pad(
            ...
            paddings=[[1, 0]],
            mode="CONSTANT",
            constant_values=args.out_bos_value,
        )
    ...
    def append_eos_fn(sample):
        return tf.pad(
            ...
            paddings=[[0, 1]],
            mode="CONSTANT",
            constant_values=args.out_eos_value,
        )
...
def process_files(args):
    ...
    pool.starmap(
        bind_process_files_chunk,
        [(i, file_chunk) for (i, file_chunk) in enumerate(files_chunks)],
    )
    t1 = time()
...
def process_files_chunk(args, i, files):
    ...
    for samples_cut in yield_cut_samples(ds):
        ...
        if len(seqs_to_write) >= args.out_records_per_file:
            out_file = (
                f"{args.out_bucket_path}/data_{i:04d}_{n_out_files:04d}.tfrecords"
            )
            ...
            with tf.io.TFRecordWriter(
                ...
                options=tf.io.TFRecordOptions(
                    compression_method=None, compression_type=""
                ),
            ) as writer:
            ...
            print(
                i,
                f"finalizing file {out_file_tmp} -> {out_file} with {n_records} records",
                flush=True,
            )
    ...
    if len(seqs_to_write) >= 100:
        ...
        with tf.io.TFRecordWriter(
            out_file_tmp,
            options=tf.io.TFRecordOptions(compression_method=None, compression_type=""),
        ) as writer:
            for record in seqs_to_write:
                write_to_file(writer, record)
