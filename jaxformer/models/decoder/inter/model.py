...
from jaxformer.utils import (
    to_f32,
    to_bf16,
    global_norm,
    pjit_noop,
    with_sharding_constraint_noop,
    create_lr_schedule_gpt3_fn,
    add_decayed_weights_exclude_ln_and_bias,
)
...
def create_model(config):
    ...
    lr_schedule_fn = create_lr_schedule_gpt3_fn(
        config["opt_warmup_steps"],
        config["opt_anneal_steps"],
        config["opt_lr_max"],
        config["opt_lr_end"],
    )
    ...
    model = TransformerDecoder(
        config=config,
        maybe_pjit=maybe_pjit,
        maybe_with_sharding_constraint=maybe_with_sharding_constraint,
        optimizer=optimizer,
    )
...
def create_transformer_fns(config, dp, mp, with_sharding_constraint, optimizer=None):
    ...
    def embedding(x):
        ...
        return EmbeddingSharded(
            in_dim=config["model_vocab_size"], out_dim=config["model_dim"]
        )(x)
    ...
    def block():
        return Block(
            ...
            d_head=config["model_head_dim"],
            d_rotary=config["model_pe_rotary_dims"],
            mp_num=mp,
            init_scale=2.0 / config["model_layers"],
        )
    ...
    def init(key, x):
        ...
        transformer_init_fn = hk.transform(
            hk.experimental.optimize_rng_use(transformer)
        ).init
        projection_init_fn = hk.transform(
            hk.experimental.optimize_rng_use(projection)
        ).init
        ...
        model = {
            ...
            "transformer": jax.lax.scan(
                init_scan_fn,
                t_key,
                xs=jax.random.uniform(t_key, input_shape, dtype=jnp.float32),
            )[1],
            "proj": projection_init_fn(
                p_key, jax.random.uniform(t_key, input_shape[1:], dtype=jnp.float32)
            ),
        }
    ...
    def create_train(shard_state_mp, shard_grad):
        def train_apply_fn(model, x, y):
            def train_loss(x, y):
                ...
                return loss(
                    vocab_size=config["model_vocab_size"], logits=logits, y=y
                ).mean()
        ...
        def train(state, x, y):
            ...
            params_bf16 = with_sharding_constraint(
                to_bf16(state["model"]), shard_state_mp
            )
            value_and_grad_fn = jax.value_and_grad(
                train_apply_fn, has_aux=False, allow_int=True
            )
            ...
            if is_single:
                ...
                grad, loss = jax.lax.scan(
                    grad_sum,
                    jax.tree_map(
                        lambda x: jnp.zeros_like(x).astype(jnp.bfloat16), params_bf16
                    ),
                    (x, y),
                )
...
class TransformerDecoder:
    def __init__(
        self, config, maybe_pjit, maybe_with_sharding_constraint, optimizer=None
    ):
        self.config = config
        ...
        def create_sharded_state(key, shard_state_fn, shard_grad_fn):
            x = jax.random.uniform(next(key), (mp * dp, 16), minval=0, maxval=1).astype(
                jnp.uint32
            )
            ...
            shard_state = {
                "step": P(),
                "model": jax.tree_map(
                    partial(shard_state_fn, parallel="mp"), state_shapes["model"]
                ),
            }

            if "opt" in state_shapes:
                shard_state["opt"] = jax.tree_map(
                    partial(shard_state_fn, parallel="mp"), state_shapes["opt"]
                )

            shard_grad = jax.tree_map(
                partial(shard_grad_fn, parallel="mp"), state_shapes["model"]
            )
            shard_state_mp = jax.tree_map(
                partial(shard_state_fn, parallel=("mp")), state_shapes["model"]
            )
        ...
        init, create_train = create_transformer_fns(
            config, dp, mp, maybe_with_sharding_constraint, optimizer
        )
        ...
        shard_state, shard_grad, shard_state_mp = create_sharded_state(
            key, shard_state_fn, shard_grad_fn
        )

        train = create_train(shard_state_mp, shard_grad)

        self.init_pjit = maybe_pjit(
            init, in_axis_resources=(None, P("pt")), out_axis_resources=shard_state
        )
        self.train_pjit = maybe_pjit(
            train,
            in_axis_resources=(shard_state, P(None, "pt"), P(None, "pt")),
            out_axis_resources=(None, shard_state, None),
            donate_argnums=(0,),
        )
        ...
        x_shape = (
            ...
            config["model_seq_len"],
        )
        x = jax.random.uniform(
            next(key), x_shape, minval=0, maxval=config["model_vocab_size"]
        ).astype(jnp.uint32)

        self.state = self.init_pjit(next(key), x)

    def train(self, sample):
        loss, self.state, grad_global_norm = self.train_pjit(
            self.state, sample["x"], sample["y"]
        )

        return (
            self.state["step"],
            np.array(loss).mean(),
            np.array(grad_global_norm).mean().astype(np.float32),
        )
    ...
    def stats(self):
        ...
        print(
            f"Model params_num_total: {params_num}, params_size_total: {params_size / 1e6:.2f}MB"
        )

        return {
            "params_num": params_num,
            "params_size": params_size,
            "params_num_total": params_num_total,
            "params_size_total": params_size_total,
        }
