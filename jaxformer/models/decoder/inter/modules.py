...
class FeedForward(hk.Module):
    def __init__(self, *, name, dim, init_stddev, ff_mult=4):
        ...
        self.proj_out = hk.Linear(
            dim,
            w_init=hk.initializers.TruncatedNormal(stddev=init_stddev / np.sqrt(dim)),
            name="proj_out",
        )
...
class Block(hk.Module):
    def __init__(self, *, name, dim, n_head, d_head, d_rotary, mp_num, init_scale=1.0):
        super().__init__(name=name)
...
class Projection(hk.Module):
    def __init__(self, n_vocab, name=None):
        ...
        self.to_logits = hk.Sequential([LayerNorm(), hk.Linear(n_vocab)])
...
class CausalSelfAttention(hk.Module):
    def __init__(
        self, *, name, dim_out, n_head, dim_head, d_rotary, init_stddev, mp_num
    ):
        super().__init__(name=name)
        ...
        self.proj_qkv = hk.Linear(
            n_head * dim_head * 3, with_bias=False, name="proj_qkv"
        )
        self.proj_out = hk.Linear(
            dim_out,
            with_bias=False,
            w_init=hk.initializers.TruncatedNormal(
                stddev=init_stddev / jnp.sqrt(dim_out)
            ),
            name="proj_out",
        )
    ...
    def __call__(self, x):
        ...
        attn_vec_sharded = attn_vec.reshape(
            attn_vec.shape[:2] + (self.mp_num, self.n_head // self.mp_num, -1)
        )
        ...
        assert (
            x_in_shape == x_out.shape
        ), f"x_in_shape={x_in_shape} != x_out_shape={x_out.shape}"
...
class EmbeddingSharded(hk.Module):
    def __init__(self, in_dim, out_dim, name=None):
        ...
        self.proj = hk.Linear(
            self.out_dim,
            w_init=hk.initializers.TruncatedNormal(stddev=1 / np.sqrt(in_dim)),
            name="proj",
        )
