...
def save_ckpt(state, path):
    ...
    with print_time(f"Saving model in {path}"):
        save_arrays(
            jax.tree_flatten(state["model"])[0],
            f"{path}/model/{jax.process_index()}.npz",
        )

    with print_time(f"Saving opt in {path}"):
        save_arrays(
            jax.tree_flatten(state["opt"])[0], f"{path}/opt/{jax.process_index()}.npz"
        )
...
def load_ckpt(state_old, path, step_overwrite=None, ignore_optimizer=False):
    ...
    with print_time(f"Loading model from {path}"):
        state_new["model"] = load_arrays(
            state_old["model"], f"{path}/model/{jax.process_index()}.npz"
        )
    ...
    if ignore_optimizer:
        ...
        with print_time(f"Loading opt from {path}"):
            state_new["opt"] = load_arrays(
                state_old["opt"], f"{path}/opt/{jax.process_index()}.npz"
            )
