...
from jaxformer.data.iterator_resumable import (
    list_files,
    create_resumable_iter,
    create_mock_iter,
    test_data_iterator,
)
...
def set_default_config(config):
    ...
    config["tpu_size_logical"] = {3: config["tpu_size"], 4: config["tpu_size"] // 2}[
        config["tpu_version"]
    ]
    ...
    config["opt_gradient_accumulation_steps"] = config.get(
        "opt_gradient_accumulation_steps", 1
    )
...
def train(config):
    ...
    if restore_ckpt is not None:
        with print_time(f"Restoring checkpoint at {restore_ckpt}/{restore_step}"):
            if restore_reinit:
                ...
                ckpt_config = master.load(
                    path=f"{restore_ckpt}/{restore_step}", step=0, ignore_optimizer=True
                )
                ckpt_step = 0
            else:
                ckpt_config = master.load(
                    path=f"{restore_ckpt}/{restore_step}", step=restore_step
                )
    ...
    if config["debug_mock_data"]:
        with print_time("Mocking dataset iterator"):
            print(
                f"Total tokens per step = {opt_gradient_accumulation_steps * opt_per_replica_batch * (tpu_size_logical // tpu_cores) * model_seq_len}"
            )
            batch_size = (
                opt_gradient_accumulation_steps,
                opt_per_replica_batch * tpu_size_logical // tpu_cores,
            )
            loader_train_iter = create_mock_iter(
                model_vocab_size, batch_size, model_seq_len
            )
            train_files = ["file"]
        ...
        with print_time("Loading dataset iterator"):
            ...
            if restore_reinit:
                ...
                loader_train_iter = create_resumable_iter(
                    train_files,
                    batch_size,
                    model_seq_len,
                    resume_at_file=None,
                ...
                if restore_ckpt is None:
                    ...
                    loader_train_iter = create_resumable_iter(
                        ...
                        batch_size,
                        model_seq_len,
                        resume_at_file=ckpt_config["data_file"],
                        resume_at_batch=ckpt_config["data_batch"],
                    )
    ...
    if config["wandb_enabled"]:
        with print_time("Initializing wandb"):
            ...
            run = wandb.init(
                ...
                name=f"{config['wandb_name']}.{config['ckpt_attempt']}",
                config=config,
                reinit=True,
            )
            wandb_run_id = run.id
    ...
    if restore_reinit or restore_ckpt is None:
        with print_time(f"Saving checkpoint at step {step}"):
            ckpt_step = master.save(
                step=step,
                path=f"{ckpt_dir}/{step}",
                ...
                wandb.log(
                    {
                        "train/loss": loss,
                        "train/lr": lr,
                        "train/grad_global_norm": grad_global_norm,
                        "perf/samples_sec": samples_sec,
                        "perf/tokens_sec": tokens_sec,
                        "perf/steps_sec": steps_sec,
                    },
                    step,
                )
            print(
                f"{timestamp} step={step:08d} lr={lr:.7f} loss={loss:.3f} data_file={data_file} data_batch={data_batch} grad_global_norm={grad_global_norm:.3f} data.shape={data.shape} steps/sec={steps_sec:.3f} samples/sec={samples_sec:.3f} tokens/sec={tokens_sec:.0f}"
            )

            if (
                step > 0
                and step % config["ckpt_every"] == 0
                or step == int(config["opt_total_steps"])
            ):
                with print_time(f"Saving checkpoint at step {step}"):
                    ckpt_step = master.save(
                        step=step,
