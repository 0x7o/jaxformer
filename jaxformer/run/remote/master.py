...
class RemoteMaster:
    ...
    def __init__(self, worker_addresses, mesh_shape, config):
        def connect_to_workers():
            def try_connect(address, attempts=10, wait_secs=10):
                for i in range(attempts):
                    try:
                        print(
                            f"Connection attempt {i} of {attempts} to worker {address}"
                        )
                ...
                raise Exception(
                    f"Can not connect to worker {address} within {attempts} attempts"
                )
    ...
    def save(self, step, path, wandb_run_id, data_files, data_file, data_batch):
        ...
        workers_process_counts = np.array(
            [worker_return[1] for worker_return in workers_return]
        )
...
def create_master(config):
    with print_time(f"Spawning TPU"):
        endpoints_ips = spawn_tpu_workers(
            tpu_user=config["tpu_user"],
    ...
    with print_time(f"Creating local worker"):
        ...
        dp = (
            config["tpu_size_logical"]
            // config["tpu_cores"]
            // config["opt_params_partitions"]
        )
        mp = config["tpu_cores"]
        mesh_shape = (dp, pt, mp)
