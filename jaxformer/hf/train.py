...
def train(args):
    ...
    if args.debug_mock_data:
        with print_time("Mocking dataset iterator"):
            print(
                f"Total tokens per step = {args.deepspeed_config['train_micro_batch_size_per_gpu'] * args.model_seq_len}"
            )
            batch_size = [1, args.deepspeed_config["train_micro_batch_size_per_gpu"]]
            loader_train_iter = create_mock_iter(
                args.model_vocab_size, batch_size, args.model_seq_len
            )
            train_files = ["file"]
    ...
    with print_time("Initializing deepspeed"):
        ...
        model_engine, optimizer, _, _ = deepspeed.initialize(
            config=args.deepspeed_config, model=model, model_parameters=model_parameters
        )
    ...
    input_ids_fixed = torch.randint(
        low=0,
        high=args.model_vocab_size,
        size=[
            args.deepspeed_config["train_micro_batch_size_per_gpu"],
            args.model_seq_len,
        ],
        dtype=torch.int64,
    ).cuda()
